...
public interface RedisZSetOps {
     ...
     */
    boolean add(KeyBuild key, Object value, double score);

    /**
     * 添加元素并设置 key 过期时间（写入 + EXPIRE 两步）。
     */
    boolean add(KeyBuild key, Object value, double score, Duration ttl);

    /**
     * 原子增加元素 score（delta 为负即减少），返回增加后的 score。
     */
    Double incrementScore(KeyBuild key, Object value, double delta);
     ...
     */
    Long rank(KeyBuild key, Object value);

    /**
     * 元素排名（score 从大到小，0 为第一）；元素不存在返回 null。
     */
    Long reverseRank(KeyBuild key, Object value);

    /**
     * 读取元素 score；元素不存在返回 null。
     */
    Double score(KeyBuild key, Object value);

    /**
     * 按下标范围读取（score 从小到大，含 end）。
     */
    <T> Set<T> range(KeyBuild key, long start, long end, Class<T> type);

    /**
     * 按下标范围读取（score 从大到小，含 end）。
     */
    <T> Set<T> reverseRange(KeyBuild key, long start, long end, Class<T> type);

    /**
     * 按下标范围读取并携带 score（score 从大到小，含 end）。
     */
    <T> Set<ZSetEntry<T>> reverseRangeWithScore(KeyBuild key, long start, long end, Class<T> type);
