...
M0 工程奠基与 M1 基础业务闭环（单库）已全部完成（2026-08-18），M2 缓存体系已收官（M2.1～M2.10 全部完成，2026-09-11；读链路最终形态：Caffeine 本地缓存 → 布隆 → Redis 逻辑过期 → DB 四级纵深）。M3 秒杀核心链路进行中：M3.1 秒杀下单 V1（纯 DB：全校验链 + 事务扣减建单）已落地，刻意保留的竞态窗口留给 M3.2 压测暴露。完整进度见 [docs/roadmap.md](docs/roadmap.md)。
