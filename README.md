...
M0 工程奠基与 M1 基础业务闭环（单库）已全部完成（2026-08-18），M2 缓存体系已收官（M2.1～M2.10 全部完成，2026-09-11；读链路最终形态：Caffeine 本地缓存 → 布隆 → Redis 逻辑过期 → DB 四级纵深）。M3 秒杀核心链路进行中（6/15）：M3.1 纯 DB 下单 V1、M3.2 压测实录竞态 + 乐观锁修复（基线 QPS 88.8/s）、M3.3+M3.4 分布式锁框架、M3.5 一人一单三层防线（10 单 → 1 单）、M3.6 库存预热 Redis + Lua 原子扣减（@ServiceLock 退役，拒绝路径 Avg 2345→113ms，DB 写从 300 次削峰到 100 次；成功路径 QPS 74.9 暴露同步建单瓶颈，M3.8 动机锚点）已落地（2026-09-16）。完整进度见 [docs/roadmap.md](docs/roadmap.md)。
