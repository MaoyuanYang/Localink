...
M0 工程奠基与 M1 基础业务闭环（单库）已全部完成（2026-08-18），M2 缓存体系已收官（M2.1～M2.10 全部完成，2026-09-11；读链路最终形态：Caffeine 本地缓存 → 布隆 → Redis 逻辑过期 → DB 四级纵深）。M3 秒杀核心链路进行中（9/15）：M3.1 纯 DB 下单 V1、M3.2 压测实录竞态 + 乐观锁修复（基线 QPS 88.8/s）、M3.3+M3.4 分布式锁框架、M3.5 一人一单三层防线（10 单 → 1 单）、M3.6 库存预热 Redis + Lua 原子扣减（拒绝路径 20 倍提速、DB 写削峰）、M3.7 MQ 框架（acks=all + 幂等生产者 + 配置化建表）、M3.8 Kafka 异步下单（成功路径 QPS 74.9 → 168、峰值吸收 847/s、Avg -81%）、M3.9 缓存失效广播、M3.10 幂等框架（@RepeatExecuteLimit 三级防护）、M3.11 消费可靠性（指数退避+耗尽回滚+超龄丢弃，回滚失败落表）已落地（2026-09-16/17）；M3.12 对账日志（Redis 流水 Lua 同原子落账 + DB 流水行随建单事务/回滚落账，traceId 三层串联）已落地（2026-09-22）。完整进度见 [docs/roadmap.md](docs/roadmap.md)。
